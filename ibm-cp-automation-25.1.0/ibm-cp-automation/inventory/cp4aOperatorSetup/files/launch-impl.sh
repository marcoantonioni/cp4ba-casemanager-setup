...
foundationCase="ibm-cp-common-services"
foundationInventory="ibmCommonServiceOperatorSetup"
...
case_depedencies="ibm-cp-common-services ibm-cert-manager ibm-bts-bundle ibm-cs-elastic ibm-cs-flink"
...
dependent_inventory_item() {
    ...
    ibm-bts-bundle)
        ...
        ;;
    ibm-cs-elastic)
        echo "elasticOperatorSetup"
        return 0
        ;;
    ibm-cs-flink)
        echo "abpFlinkOperatorSetup"
        return 0
        ;;
    *)
...
# Verfiy arguments are valid
   ...
   # sed <"${casePath}"/inventory/"${inventory}"/files/op-olm/subscription.yaml "s|REPLACE_NAMESPACE|${namespace}|g" | sed "s|REPLACE_CHANNEL_NAME|$channelName|g" | $kubernetesCLI apply -n "${namespace}" -f -
    sub_inst_list=$(oc get subscription -n $NAMESPACE|grep ibm-cp4a-operator|awk '{if(NR>0){if(NR==1){ arr=$1; }else{ arr=arr" "$1; }} } END{ print arr }')
    ...
    if [[ -z "${sub_array[i]}" ]]; then
        ...
        for i in ${!sub_array[@]}; do
            oc patch subscription ${sub_array[i]} -n $NAMESPACE -p '{"spec":{"channel":"'"$channelName"'"}}' --type=merge
            if [ $? -eq 0 ]; then
                echo "Update the channel of subsciption '${sub_array[i]}' to $channelName!"
            else
                echo "Failed to update the channel of subsciption '${sub_array[i]}' to $channelName! exiting now..."
                exit 1
            fi
        done
    fi
